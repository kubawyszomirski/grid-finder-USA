...
def build_report(
    ...
    lines = [
        ...
        "",
        f"  Label balance (train):  {y_train.value_counts().to_dict()}",
        f"  Label balance (val)  :  {y_val.value_counts().to_dict()}",
        f"  Positive rate  train :  {y_train.mean():.3f}",
        f"  Positive rate  val   :  {y_val.mean():.3f}",
        "",
        "-" * 62,
        ...
        "-" * 62,
        correlation_report(X_train, y_train),
    ]
    return "\n".join(lines)
...
def preprocess(
    ...
    Full pipeline:
      ...
      5. RobustScaler fit on training, applied to val + predict (optional)
    ...
    meta_train = df_train[[c for c in ("tile_id", "state") if c in df_train.columns]].copy()
...
def main() -> None:
    ...
    parser.add_argument(
        "--val-frac", type=float, default=VAL_FRACTION,
