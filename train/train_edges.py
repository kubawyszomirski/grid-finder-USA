...
Writes to {project}/models_edges/:
    edges_model_{N}.ubj          — XGBoost model (binary format)
    edges_model_{N}_meta.json    — training config, val metrics, feature importances
    edges_model_{N}_val_preds.parquet — val set edge_id + y_true + y_proba

Models are auto-numbered starting at 0 (next available index).

Usage:
    python train_edges.py --project project-ma
    python train_edges.py --project project-ma --max-depth 4 --learning-rate 0.02
"""
...
# XGBoost defaults tuned for edge data (sparser, more structured than roads)
XGB_DEFAULTS = dict(
    n_estimators          = 5000,
    learning_rate         = 0.02,
    max_depth             = 3,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train XGBoost on preprocessed edge features.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python train_edges.py --project project-ma\n"
            "  python train_edges.py --project project-ma --max-depth 4 --n-estimators 3000\n"
        ),
    )
    ...
    if any(x is None for x in [X_train, y_train, X_val, y_val]):
        print("[ERROR] Missing preprocessed splits — run preprocess_edges.py first.")
        return
    ...
    # Load meta for val edge_ids (optional — for output parquet)
    ...
    if meta_val is not None and "edge_id" in meta_val.columns:
        val_preds.insert(0, "edge_id", meta_val["edge_id"].values)
